...
//var rpp_resolve = require('rpp/lib/resolve'); // nope! not with node v0.10
var rpp_resolve = require(path.join(path.dirname(require.resolve('rpp')), 'resolve.js'));
var rpp_isModule = require(path.join(path.dirname(require.resolve('rpp')), 'isModule.js'));
                ...
                return "(new (require('rpp/lib/Collection'))({" + Object.keys(filenames).map(function(filename){
                        return "'" + filename + "': " + (
                            rpp_isModule(filenames[filename])
                                ? "require('" + filename + "')"
                                : "require('fs').readFileSync(require('path').join(__dirname, '" + filename + "'), 'utf8')"
                        );
                    }).join(', ') + "}))";
